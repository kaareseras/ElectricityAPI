...
async def get_hardware_by_id(pk: int, session: Session = Depends(get_db_session), user=Depends(get_current_admin)):
...
@hardware_router.get("", status_code=status.HTTP_200_OK, response_model=list[HardwareResponse])
async def get_hardware(session: Session = Depends(get_db_session), user=Depends(get_current_admin)):
