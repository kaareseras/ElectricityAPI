...
@chargeowner_router.get("/with-latest-charge", response_model=list[ChargeownerLatestCharge])
async def fetch_chargeowner_with_latest_charge(
    session: Session = Depends(get_db_session), user=Depends(get_current_user)
):
    return await chargeowner.fetch_chargeowner_with_latest_charge(session)


@chargeowner_router.get("/{pk}", status_code=status.HTTP_200_OK, response_model=ChargeownerResponse)
async def get_chargeowner_info(pk, session: Session = Depends(get_db_session), user=Depends(get_current_admin)):
...
async def add_new_chargeowner(
    data: AddChargeownerRequest, session: Session = Depends(get_db_session), user=Depends(get_current_admin)
):
...
async def delete_chargeowner(pk: int, session: Session = Depends(get_db_session), user=Depends(get_current_admin)):
