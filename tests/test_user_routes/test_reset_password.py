...
from src.fastapi_app.utils.email_context import FORGOT_PASSWORD
...
def test_reset_password_invalid_token(client, user):
    data = {"token": "sakjdhajksdhaksjhdjkadh", "email": user.email, "password": NEW_PASSWORD}
    response = client.put("/auth/reset-password", json=data)
...
def test_reset_password_invalid_email_format(client, user):
    data = {"token": _get_token(user), "email": "describly.com", "password": NEW_PASSWORD}
    response = client.put("/auth/reset-password", json=data)
...
def test_reset_password_nonexistent_email(client, user):
    data = {"token": _get_token(user), "email": "nandan@describly.com", "password": NEW_PASSWORD}
    response = client.put("/auth/reset-password", json=data)
    assert response.status_code == 400
    del data["token"]
    del data["email"]
